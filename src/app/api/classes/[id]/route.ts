...
const classUpdateSchema = z.object({
  ...
  startDate: z.string().refine((val) => !isNaN(Date.parse(val)), 'تاریخ شروع نامعتبر است'),
  endDate: z.string().refine((val) => !isNaN(Date.parse(val)), 'تاریخ پایان نامعتبر است'),
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'خطا در دریافت اطلاعات کلاس' }, { status: 500 });
  }
}